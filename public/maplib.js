...
(function () {
    ...
    var MarkerClusterer =
        ...
        BMapLib.MarkerClusterer = function (map, options) {
            ...
            isArray(mkrs) && this.addMarkers(mkrs, _data, _click);
        };
     ...
     * @return 无返回值。
     */
    MarkerClusterer.prototype.addMarkers = function (markers, _data, _click) {
        for (var i = 0, len = markers.length; i < len; i++) {
            this._pushMarkerTo(markers[i], _data, _click, i);
        }
     ...
     * @return 无返回值。
     */
    MarkerClusterer.prototype._pushMarkerTo = function (marker, _data, _click, i) {
        var index = indexOf(marker, this._markers);
        if (index === -1) {
            marker.isInCluster = false;
            marker._click = _click;
            marker._data = _data[i];
    ...
    Cluster.prototype.addLabel = function (marker) {
        ...
        if (marker._click) {
            label.addEventListener('click', () => { marker._click(marker._data) });
        } else {
